...
#' @return \code{pscores}: Propensity scores (4 columns): under treatment in period 1, under treatment in period 0, under control in period 1, under control in period 0.
#' @return \code{outcomepred}: Conditional outcome predictions (3 columns): in treatment group in period 0, in control group in period 1, in control group in period 0.
...
didDML<-function(y, d, t, x, MLmethod="lasso", est="dr",  trim=0.05, cluster=NULL, k=3){
  ...
  trimmed=1*((param[,6]<trim*d*(1-t)) | (param[,7]<trim*(1-d)*t) | (param[,8]<trim*(1-d)*(1-t)))     # identify observations with small propensity scores in subgroups which are dropped
  ...
  list(ATET=ATET, se=se, pval=pval, ntrimmed=sum(trimmed), pscores=param[,5:8], outcomepred=param[,9:11])
}