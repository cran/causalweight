...
#' @return \code{pscores}: Propensity scores (2 columns): under treatment, under control.
#' @return \code{outcomepred}: Conditional outcome predictions.
...
list(ATET=ATET, se=se, pval=pval, ntrimmed=sum(trimmed), pscores=param[,4:5], outcomepred=param[,6])
}