...
#' @param trim Trimming rule for discarding observations with extreme generalized propensity scores. Observations with f(D=\code{d1}|M,X)<\code{trim} or f(D=\code{d0}|M,X)<\code{trim} are dropped, with f denoting the generalized propensity score (or conditional density of treatment).
...
#' output=medweightcont(y,d,m,x, d0=0, d1=1, ATET=FALSE, trim=0.05, lognorm=FALSE, bw=NULL, boot=19)
#' round(output$results,3)
#' output$ntrimmed
#' @importFrom stats binomial fitted.values glm lm pnorm sd rnorm dnorm
